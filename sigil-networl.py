...
class EnhancedVWPRWrapper:
    ...
    def generate_sigil(self, layer_name: str, epoch: int, 
        ...
        # For layers with many nodes, use a sampling strategy
        if num_nodes > 1000:
        ...
        if archetype == "The Sentinel":
            # Circular layout with strong organization
            ...
            # Default spiral layout that becomes more organized with higher resilience
            theta = np.linspace(0, 4*np.pi, num_nodes, endpoint=False)
            r = theta / (4*np.pi) * (1.5 - ρ_l/4)  # More resilient = tighter spiral
            x = r * np.cos(theta)
            y = r * np.sin(theta)
        ...
        if archetype in ["The Oracle", "The Sentinel", "The Archivist"] and num_nodes < 500:
            for i in range(num_nodes):
                for j in range(i+1, min(i+10, num_nodes)):
                    # Connect nodes that are close in space and value
        ...
        for i, phi_val in enumerate(phi_sampled):
            ...
            # Vary node shape based on archetype
            if archetype == "The Sentinel":
            ...
            # Add crack lines for certain archetypes
        ...
        if archetype == "The Sentinel":
            # Add concentric rings based on resilience
    ...
    def sigil_gallery(self, total_epochs=100, interval=10, out_dir="sigil_gallery", 
        ...
        # Create a visualization of the summary
    ...
    def create_summary_visualization(self, out_dir):
        ...
        try:
            ...
            # Plot each metric over epochs, grouped by layer
            ...
            for i, metric in enumerate(['Luminosity', 'FractureDensity', 'Entropy', 'Resilience']):
                ...
                if i == 0:  # Only put legend on first plot
