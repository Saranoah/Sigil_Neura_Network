...
from .layer import SigilLayer

class SigilNetworkManager:
    """
    SigilNetworkManager
    ===================
    Orchestrates the lifecycle of a Sigil-based model:
      - Epoch tracking
    ...
    def record_epoch(self):
        """Record the phi state for all Sigil layers."""
    ...
    def generate_sigil(self, layer, epoch, dpi=120):
        """
        Create a visual sigil for a given layer.
        Each sigil reflects the current state of `phi` values.
        """
        ...
        H_l = entropy(hist + 1e-12) / np.log(len(hist))  # Entropy
    ...
    def generate_gallery(self, epoch):
        """Generate sigil images for all Sigil layers."""
        gallery_log = []
        for layer in self.sigil_layers:
    ...
    def summary_metrics(self):
        """Collect and return all network metrics."""
        metrics = []
        for layer in self.sigil_layers:
